...
namespace BestHTTP.SignalRCore
{
    public sealed class HubConnection
    {
        ...
        {
            return string.Format("{{'protocol':'{0}'}}", this.Protocol.Encoder.Name);
        }
        
        private void TransportData(string data)
        {
            this.messages.Clear();
            try
            {
                this.Protocol.ParseMessages(data, ref this.messages);
            }
            catch (Exception ex)
            {
                HTTPManager.Logger.Exception("HubConnection", "ParseMessage(string) failed.", ex);
            }

            for (int i = 0; i < this.messages.Count; ++i)
                this.MessageArrived(this.messages[i]);

            this.messages.Clear();
        }

        private void TransportData(byte[] data)
        {
            this.messages.Clear();
            try
            {
            ...
            {
                HTTPManager.Logger.Exception("HubConnection", "ParseMessage(byte) failed.", ex);
            }
        ...
        private void Transport_OnStateChanged(TransportStates oldState, TransportStates newState)
        {
            ...
            switch (newState)
            {
                case TransportStates.Connected:
                    ...
                    this.Transport.OnTextData += TransportData;
                    this.Transport.OnBinaryData += TransportData;
                    break;
